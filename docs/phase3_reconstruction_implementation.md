...
| Sections 7 and 8: method-independent seasonal metrics and peak reliability | `reconstruction_metrics.py` applies one definition to all methods: global common-support maximum, contiguous-plateau midpoint, non-contiguous equal-maximum ambiguity, boundary flags, peak magnitude, segment-wise trapezoidal integral, Pearson correlation, and 5/10/15-day flags. A reference peak at either support boundary retains its descriptive date difference but receives an unavailable primary timing status and no reliability classification. | Tests cover interior peaks, first/last-day reference peaks, plateau midpoint, ambiguity, boundary status, continued eligibility of other metrics, disconnected integration, and the frozen timing thresholds. No onset/end or secondary-event detector exists. |
...
point. It refuses to run unless `--execute-performance` is supplied and the
complete Git worktree is clean, verifies the stored manifest checksum,
regenerates all gates against the current input and TIMESAT runtime, and
requires an exact manifest match before making the first reconstruction call.
It was not executed for this implementation task.
